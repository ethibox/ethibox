...
import { App } from './orm.js';
import { DEFAULT_LOCALE, TEMPLATES_URL, EMAIL_BLOCKLIST } from './constants.js';

const en = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'public/locales/en/common.json'), 'utf8'));
const fr = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'public/locales/fr/common.json'), 'utf8'));
...
export const generateReleaseName = async (name) => {
    if (!/^[a-zA-Z0-9_-]+$/.test(name)) throw new Error('Invalid release name');
...
export const fetchTemplates = async (enabled = true, refresh = false) => {
    const templatesFile = path.join(process.cwd(), 'data', 'templates.json');

    if (!fs.existsSync(templatesFile) || refresh) {
    ...
    templates = templates.map(({ title, categories, enabled: _, ...template }) => ({
        stackfile: `${template.repository.url.replace('github.com', 'raw.github.com')}/${template.repository.commit || 'master'}/${template.repository.stackfile}`,
        category: Array.isArray(categories) ? categories[0] ?? null : null,
        commit: template.repository.commit,
        name: title,
        ...template,
    }));
