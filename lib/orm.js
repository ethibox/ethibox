...
import { STATE, SENSITIVE_ENVS } from './constants.js';
...
export const App = sequelize.define('app', {
    ...
    name: {
        ...
        get() {
            return this.getDataValue('releaseName')?.replace(/\d+$/, '').toLowerCase();
        },
        set() {
...
export const Env = sequelize.define('env', {
    ...
    hooks: {
        beforeSave: (env) => {
            ...
            if (env.changed('value') && name && SENSITIVE_ENVS.test(name)) {
